...
Triggers when you push a version tag (e.g., `v1.5.3`):
...
5. **Check the release**:
   - Go to Releases tab
   - You'll see "Codentis v1.5.3" with all installers attached
...
### Artifacts Produced
- `Codentis-Setup-1.5.3.exe` (Windows installer, ~35 MB)
- `Codentis-1.5.3-arm64.pkg` (macOS Apple Silicon, ~30 MB)
- `Codentis-1.5.3-intel.pkg` (macOS Intel, ~30 MB)
- `codentis_1.5.3_amd64.deb` (Linux Debian/Ubuntu, ~30 MB)
...
### Release Not Created
- Make sure you pushed the tag: `git push origin v1.5.3`
- Check that tag starts with `v` (e.g., `v1.5.3` not `0.1.1`)
...
on:
  push:
    tags:
      - 'v*'  # Current: triggers on v1.5.3, v1.5.3, etc.
...
2. ✅ Test with a pre-release tag: `v1.5.3-beta`
