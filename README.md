...
**Windows:**
```bash
# Download and run Codentis-Setup-1.4.3.exe
# The installer will add Codentis to your PATH
```

**macOS:**
```bash
# Download Codentis-1.4.3-arm64.pkg (Apple Silicon) or Codentis-1.4.3-intel.pkg (Intel)
sudo installer -pkg Codentis-1.4.3-arm64.pkg -target /
```

**Linux (Debian/Ubuntu):**
```bash
# Download codentis_1.4.3_amd64.deb
...
# Creates: dist/Codentis.exe and dist/Codentis-Setup-1.4.3.exe
```
...
# Creates: dist/codentis and dist/Codentis-1.4.3-arm64.pkg (or -intel.pkg)
```
...
# Creates: dist/codentis and dist/codentis_1.4.3_amd64.deb
```
...
- **Release**: Triggered by version tags (e.g., `v1.4.3`) to create releases with installers
