...
export default function Features() {
    ...
    return (
        <section id="features" className="py-24 sm:py-32 sm:py-40 relative" ref={sectionRef}>
            <div className="max-w-[1120px] mx-auto px-4 sm:px-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {features.map((f) => (
                        <div
                            key={f.num}
                            className={`reveal card-glow bg-bg-2 border border-white/[0.05] rounded-2xl p-6 sm:p-9 relative overflow-hidden group ${f.highlight
                                    ? "sm:col-span-2 lg:col-span-3 lg:grid lg:grid-cols-2 lg:gap-10 lg:items-center"
                                    : ""
                                }`}
                        >
                            <div className="relative z-[1]">
                                <div className="font-mono text-[0.62rem] sm:text-[0.68rem] font-medium text-cyan-500/60 mb-3 sm:mb-4 tracking-wider">
                                    {f.num}
                                </div>
                                <h3 className="text-base sm:text-lg font-semibold mb-2 sm:mb-2.5 tracking-tight text-white/90 group-hover:text-white transition-colors duration-300">
                                    {f.title}
                                </h3>
                                <p className="text-[0.82rem] sm:text-[0.88rem] text-zinc-500 leading-relaxed group-hover:text-zinc-400 transition-colors duration-300">
                                    {f.desc}
                                </p>
                            </div>
